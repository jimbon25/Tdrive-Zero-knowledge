...
export default function OmniCloudFilesPage({ params }: { params: { path?: string[] } }) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const currentPathSegments = params.path || [];
  const virtualPath = "/" + currentPathSegments.join("/");
  
  const { data: files, isLoading, error, refetch } = useFiles(virtualPath, "omnicloud");
  ...
  const handleDelete = async (file: FileType) => {
    ...
    if (isConfirmed) {
      ...
      queryClient.invalidateQueries({ queryKey: ["files", virtualPath, "omnicloud"] });
  ...
  const createFolderMutation = useMutation({
    mutationFn: async (name: string) => {
      const response = await api.post("/files/folder", { name, vpath: virtualPath, storage_provider: "omnicloud" });
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-32 text-neutral-400">
          ...
          <p className="text-sm font-medium animate-pulse">Syncing with OmniCloud...</p>
        </div>
      ) : error ? (
        ...
        <div className="flex flex-col items-center justify-center py-32 bg-neutral-50/50 dark:bg-neutral-900/20 border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-3xl text-neutral-400">
          ...
          <p className="text-sm">Upload a file or create a folder in OmniCloud.</p>
