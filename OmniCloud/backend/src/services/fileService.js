...
export function replaceFilesForAccount(userId, cloudAccountId, records) {
	// Query existing file metadata to preserve their IDs
	...
	const existingIdMap = new Map(
		existingFiles.map((file) => [file.remote_file_id, file.id])
	);

	const normalizedRecords = records.map((record) => ({
		id: record.id || existingIdMap.get(record.remote_file_id) || randomUUID(),
		user_id: userId,
		virtual_path: normalizePath(record.virtual_path),
		file_name: record.file_name,
		is_folder: record.is_folder ? 1 : 0,
